...
from services.matching.async_openai_embedding_matcher import EmbeddingMatcher
...
async def recommend_products(query: str, products: List[Product]) -> List[RecommendedProduct]:
    ...
    all_ranked=await matcher.rank(query,products)
    try:
        # raise Exception("An intentional, Testing generic exception so fall back can be executed")
        ranked = all_ranked
